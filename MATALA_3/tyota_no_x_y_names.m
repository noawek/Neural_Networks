...
for k = 1:numel(S)
    ...
    if length(spiketrain_before) == length(spiketrain)   
        ...
        % loading some relevant data from them before continuing:
        POSY{k+1} = posy;
        POSX{k+1} = posx;
...
for i = 1:size(rateperpos,1)
    figure('Units','normalized','position',[0 0 1 1]);
    hold on;
    imagesc(imgaussfilt(squeeze(rateperpos(i,:,:)))); 
    xlim([0 21]);
    ylim([0 21]);
    axis square;
    colormap("hot");
...
for i = 1:size(rateperpos_fake,1)
    figure('Units','normalized','position',[0 0 1 1]);
    hold on;
    sgtitle({['\fontsize{14} \bf Neuron ' num2str(i) ' -'], '\fontsize{12} \rm Superimposing the Spatial Activity of the Neuron [Action Potentials] on the Spatial Activity of the Rat [her track in a 100cmX100cm squared environment]'});
    subplot(1,3,1);
    hold on;
    plot(POSX{i},POSY{i});  
    ...
    figure('Units','normalized','position',[0 0 1 1]);
    hold on;
    sgtitle({['\fontsize{14} \bf Neuron ' num2str(i) ' -'], '\fontsize{12} \rm Firing Rate [Hz] as a function of the Location of the rat [100cmX100cm squared environment]'});
    subplot(1,3,1);
    imagesc(imgaussfilt(squeeze(rateperpos(i,:,:)))); 
    xlim([0 21]);
    ylim([0 21]);
    ...
    title('Original Data');
    subplot(1,3,2);
    imagesc(imgaussfilt(squeeze(rateperpos_fake(i,:,:)))); 
    xlim([0 21]);
    ylim([0 21]);
    axis square;
    colormap("hot");
    ...
    title('Randomly Permuted Data 1');
    subplot(1,3,3);
    imagesc(imgaussfilt(squeeze(rateperpos_fake2(i,:,:)))); 
    xlim([0 21]);
    ylim([0 21]);
    axis square;
    colormap("hot");
