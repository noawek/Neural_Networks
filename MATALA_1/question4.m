...
for j = CurrentEndTime:(length(T)-CurrentDuration)
    I_inj =  zeros(size(T), 'like', T);
    I_inj([CurrentStartTime:CurrentEndTime, (length(T)-(j+CurrentDuration)):(length(T)-j)]) = CurrentInj;
    n(1) = n0;
    m(1) = m0;
    h(1) = h0;
    V(1) = V_rest;
...
if TimesCrossing == 2
    figure('Name', 'REFRACTORY', 'Units','normalized','position',[0 0 1 1]);
    hold on;
    subplot(4,2,1:3,1);
    plot(T, V);
...
elseif TimesCrossing == 1 && end_ref == 0 
       end_ref = T(j+1);
       subplot(4,2,1:3,2);
       plot(T, V);
       title('Voltage VS Time');
